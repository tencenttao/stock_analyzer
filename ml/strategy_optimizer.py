...
逻辑概要:
  1. 数据: data/quarterly_data 下季度 JSON，按季度训练、按月评估。
...
数据流:
  1. 特征来源: 全部来自已生成的季度 JSON（data/quarterly_data/*_ml_training_data.json）。
...
def run_optimization(start_year=2019):
    """运行优化实验"""
    print("=" * 80)
    print("策略优化实验")
    print("=" * 80)
    ...
    if not quarterly_data:
        print("无季度数据，请检查 data/quarterly_data 目录")
        return [], None, None
    ...
    return experiments, best_config, best_result


def run_ensemble_experiment(start_year=2019):
    """集成策略实验"""
    print()
    print("=" * 80)
    print("集成策略实验（多模型投票）")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--start-year', type=int, default=2025, help='回测起始年份（默认2019，多年评估）')
    parser.add_argument('--ensemble', action='store_true', help='运行集成实验')
    args = parser.parse_args()
    
    experiments, best_config, best_result = run_optimization(start_year=args.start_year)
    
    if args.ensemble:
        run_ensemble_experiment(start_year=args.start_year)
